        ...
        <thead>
          <tr className="text-center">
           ...
           <tr v-for="(cartItem, index) in cartItems" :key="index" class="text-center">
            <td>{{ index+1 }}</td>
               <td><img class="rounded-circle" :src="'http://localhost:8000/images/'+ cartItem.product.image" ></td>
             <td>{{ cartItem.product.name }}</td>
          <td>{{ cartItem.product.price }} LE</td>
          <td>{{ cartItem.quantity }}</td>
          <td>{{ cartItem.product.price * cartItem.quantity }} LE</td>
             
             <td> <button
  @click="incrementCartItem(cartItem.product.id)"
...
<td>
<button
  @click="decrementCartItem(cartItem.product.id)"
...
export default {
 ...
 const calculateTotalPrice = () => {
  ...
  return totalPrice;
    ...
    //   axios.put(`http://localhost:7000/api/cart/increment-item/${userId}/${product}`)
    //     .then((res) => {
...
const incrementCartItem = (productId) => {
  ...
  if (cartItem) {
    ...
    axios.put(`http://localhost:8000/api/cart/increment-item/${userId}/${productId}`)
      .then((res) => {
...
const decrementCartItem = (productId) => {
  ...
  if (cartItem) {
    ...
    // Send a request to update the cart item on the server
    axios.put(`http://localhost:8000/api/cart/decrement-item/${userId}/${productId}`)
      .then((res) => {
        console.log(res);
      })
      .catch((err) => {
...
const removeCartItem = (productId) => {
  ...
  if (itemIndex !== -1) {
    ...
    axios.delete(`http://localhost:8000/api/cart/remove-item/${userId}/${productId}`)
      .then((res) => {
        // Handle the response as needed
        console.log(res);
      })
